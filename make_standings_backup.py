...
class _Caches:
    def __init__(self, df_ind, df_standings):
        self.df_ind = df_ind
        self.df_st = df_standings

        # Quick flags
        self._div_mask  = df_ind["div_game"].to_numpy()
        self._conf_mask = df_ind["conf_game"].to_numpy()

        # Opponent sets per team (for "common opponents")
        self.oppsets = (df_ind.groupby("Team")["Opponent"]
                        .agg(lambda s: set(s.tolist()))
                        .to_dict())

        # Victory sets per team (for "strength of victory")
        self.victory_sets = (df_ind.loc[df_ind["Outcome"] == "Win"]
                             .groupby("Team")["Opponent"]
                             .agg(lambda s: set(s.tolist()))
                             .to_dict())

        # Head-to-head mean outcome points matrix (Team x Opponent)
        self.h2h = (df_ind.pivot_table(index="Team", columns="Opponent",
                                       values="Outcome_points", aggfunc="mean"))

        # Overall WLT (mean of outcome points)
        self.wlt_all = df_ind.groupby("Team")["Outcome_points"].mean()

        # WLT in division / conference
        self.wlt_div  = (df_ind.loc[self._div_mask]
                         .groupby("Team")["Outcome_points"].mean())
        self.wlt_conf = (df_ind.loc[self._conf_mask]
                         .groupby("Team")["Outcome_points"].mean())

        # Points scored / allowed totals (overall + conference)
        sums = df_ind.groupby("Team")[["Points_scored","Points_allowed"]].sum()
        self.ps = sums["Points_scored"]
        self.pa = sums["Points_allowed"]
        conf_sums = (df_ind.loc[self._conf_mask]
                     .groupby("Team")[["Points_scored","Points_allowed"]].sum())
        self.ps_conf = conf_sums["Points_scored"]
        self.pa_conf = conf_sums["Points_allowed"]

        # Ranks for points scored/allowed precomputed once (overall & by conf)
        self.conf_of = pd.Series({t: div_series[t][:3] for t in div_series.index})
        self.rank_ps = (-self.ps).rank(method="min")                # higher points -> lower rank number
        self.rank_pa = ( self.pa).rank(method="min")                # lower allowed -> lower rank number
        # per-conference ranks
        self.rank_ps_conf = {}
        self.rank_pa_conf = {}
        for conf in ["AFC","NFC"]:
            idx = [t for t in self.df_st.index if self.conf_of[t] == conf]
            self.rank_ps_conf[conf] = (-self.ps.loc[idx]).rank(method="min")
            self.rank_pa_conf[conf] = ( self.pa.loc[idx]).rank(method="min")
...
# FAST replacement for make_ind + outcome computation
def make_ind(df):
    if "schedule_playoff" in df.columns:
        df = df.loc[~df["schedule_playoff"]].copy()

    # Build the 2-row-per-game long form via concat (avoids iloc slicing)
    ...
    # Categorical dtypes speed up groupby/joins a lot
    df_ind["Team"]     = pd.Categorical(df_ind["Team"], categories=sorted(div_series.index))
    df_ind["Opponent"] = pd.Categorical(df_ind["Opponent"], categories=sorted(div_series.index))

    # Numeric outcome points: Win=1, Tie=0.5, Loss=0 (avoids value_counts in get_WLT)
    ps, pa = df_ind["Points_scored"].to_numpy(), df_ind["Points_allowed"].to_numpy()
    p = (ps > pa).astype(float)
    p[ps == pa] = 0.5
    df_ind["Outcome_points"] = p

    # Keep string Outcome if you still need it elsewhere
